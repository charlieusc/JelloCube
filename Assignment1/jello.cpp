...
void display()
{
  ...
  // camera parameters are Phi, Theta, R
    lookAtEye = {R * cos(Phi) * cos (Theta), R * sin(Phi) * cos (Theta), R * sin (Theta)};
    
    gluLookAt(lookAtEye.x, lookAtEye.y, lookAtEye.z,
              lookAtDes.x, lookAtDes.y, lookAtDes.z,
  ...
  */
  ...
  showBoundingBox();
    //showAxis();
    showInlinedPlane(&jello, drawSet);
  glutSwapBuffers();
}
...
void doIdle()
{
  ...
  */
    ...
    if (sprite<300 && saveScreenToFile==1 && slower==5) {
        saveScreenshot(windowWidth, windowHeight, s);
        sprite++;
        slower =0;
    }
...
int main (int argc, char ** argv)
{
  ...
  calculateDrawingPoint(&jello, drawSet);
    lookAtDes = {0,0,0};
    lookAtUp ={0,0,1};
