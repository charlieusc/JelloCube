...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iRightMouseButton) // handle camera rotations
  {
    ...
    if (g_iLeftMouseButton) {
        ...
        msForceHa = crossProduct(EDir, lookAtUp);
        msForceUp.z = -vMouseDelta[1]*msforceScale;
        msForceHa.x *= vMouseDelta[0]*msforceScale*0.1;
        msForceHa.y *= vMouseDelta[0]*msforceScale*0.1;
        msForceHa.z *= vMouseDelta[0]*msforceScale*0.1;
